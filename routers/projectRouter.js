...
router.post("/", async (req, res) => {
    ...
    if (!project.name || !project.description) {
        res.status(400).json({ message: "Please enter a valid project name and description" });
    } else {
        try {
            ...
            res.status(500).json({ message: `Your project could not be posted ${error}.` });
        }
    }
});
...
router.delete("/:id", async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: `The project's information could not be modified: ${error}.`
        });
    }
});
